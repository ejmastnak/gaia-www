...
<template>
  <div class="relative w-fit">
    <Listbox as="div" :modelValue="locale" @update:modelValue="switchLanguageTo">
      ...
      <div class="relative">
        <ListboxButton class="inline-flex divide-x divide-orange-700 rounded-md outline-none">
          <div class="inline-flex items-center gap-x-1.5 rounded-l-md bg-orange-700 px-3 py-2 text-white">
            <GlobeAltIcon class="-ml-0.5 size-5" aria-hidden="true" />
            <p class="text-sm font-semibold">{{locale}}</p>
          </div>
          <div class="inline-flex items-center rounded-l-none rounded-r-md bg-orange-700 p-2 outline-none hover:bg-orange-800 focus-visible:outline focus-visible:outline-2 focus-visible:outline-orange-400">
        ...
        <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
          <ListboxOptions class="absolute right-0 z-10 mt-2 w-40 origin-top-right divide-y divide-gray-200 overflow-hidden rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
            <ListboxOption as="template" v-for="(option, idx) in supportedLocales" :key="`locale-${option}`" :value="option" v-slot="{ active, selected }">
              <li :class="[active ? 'bg-orange-700 text-white' : 'text-gray-900', 'cursor-default select-none p-4 text-sm']">

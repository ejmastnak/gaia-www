export default {
  index: {
    h1: {
      en: "Products",
      sl: "Izdelki",
    },
    summary: {
      en: `We specialize in apple juice, apple cider vinegar, and herbal products made on our farm at small scale with home-grown ingredients.
Our products are free of pesticides, herbicides, and all other synthetic agrochemicals.
`,
      sl: "Nekaj po slovensko",
    },
  },
  jabsok: {
    name: {
      en: "Apple Juice",
      sl: "Jabolčni sok",
    },
    summary: {
      en: `
Our apple juice is pressed in small batches each fall from apples grown in our tall-trunk meadow orchards.
Our juice is intentionally unfiltered to preserve body and flavor, which, together with the diverse combination of heirloom varieties that go into our juice, gives it a depth of flavor and richness not found in commercial products.
`,
      sl: "Nekaj po slovensko",
    },
    h1: {
      en: "Apple Juice",
      sl: "Jabolčni sok",
    },
  },
  jabkis: {
    name: {
      en: "Apple Cider Vinegar",
      sl: "Jabolčni kis",
    },
    summary: {
      en: `
Our production of apple cider vinegar goes hand in hand with the pressing of apple juice each fall—we make our vinegar from the pulp and residual juice left over from the pressing process.
The relatively low sugar level and high concentration of apple solids in the raw ingredients, together with a slow fermentation all winter long, gives our vinegar a smoothness, gentleness, and complexity not found in conventional vinegar made only from fermented apple juice.
`,
      sl: "Nekaj po slovensko",
    },
    h1: {
      en: "Apple Cider Vinegar",
      sl: "Jabolčni kis",
    },
  },
  sntolj: {
    name: {
      en: "Saint John's Wort Oil",
      sl: "Šentjanževo olje",
    },
    summary: {
      en: `
      Saint John's Wort oil is a traditional Slovene herbal product made from the flowers of the Saint John's Wort plant.
      It is used to sooth and heal damaged skin after minor burns, to improve circulation at the skin surface, and as a luxurious massage oil.
      We make our oil by steeping St. John's Wort flowers—hand-picked from meadows on our mountain pastures—in cold-pressed olive oil over multiple weeks, and then filtering out the resulting crimson-colored oil. 
`,
      sl: "Nekaj po slovensko",
    },
    h1: {
      en: "St. John's Wort Oil",
      sl: "Šentjanževo olje",
    },
  },
}
